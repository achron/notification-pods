...
@objc public class ProductDisplayVC: BaseNotificationContentVC, UIGestureRecognizerDelegate {
    ...
    @objc func smallImageAction(_ sender: UITapGestureRecognizer) {
        ...
        case 1:
            ...
            self.titleLabel.text = jsonContent?.btnTitle1
            self.subTitleLabel.text = jsonContent?.btnSubTitle1
            let priceText = "₹ " + (jsonContent?.price1 ?? "")
            self.priceLabel.text = priceText
            self.deeplink = jsonContent?.deepLink1 ?? ""
            break
        case 2:
            self.bigImageView.image = smallImageBtn2.image
            self.titleLabel.text = jsonContent?.btnTitle2
            let priceText = "₹ " + (jsonContent?.price2 ?? "")
            self.priceLabel.text = priceText
            self.subTitleLabel.text = jsonContent?.btnSubTitle2
            self.deeplink = jsonContent?.deepLink2 ?? ""
            break
        case 3:
            self.bigImageView.image = smallImageBtn3.image
            self.titleLabel.text = jsonContent?.btnTitle3
            self.subTitleLabel.text = jsonContent?.btnSubTitle3
            let priceText = "₹ " + (jsonContent?.price3 ?? "")
            self.priceLabel.text = priceText
            self.deeplink = jsonContent?.deepLink3 ?? ""
            break
        default:
            break
        }
    ...
    func createView() {
        ...
        Utiltiy.checkImageUrlValid(imageUrl: jsonContent.img1) { [weak self] (imageData) in
        ...
        Utiltiy.checkImageUrlValid(imageUrl: jsonContent.img2) { [weak self] (imageData) in
            DispatchQueue.main.async {
                if imageData != nil {
        ...
        if let img3 = jsonContent.img3, !img3.isEmpty ,(jsonContent.btnTitle3 != nil && jsonContent.btnSubTitle3 != nil && jsonContent.deepLink3 != nil && jsonContent.price3 != nil){
            Utiltiy.checkImageUrlValid(imageUrl: img3) { [weak self] (imageData) in
                DispatchQueue.main.async {
                    if imageData != nil {
        ...
        let priceText = "₹ " + (jsonContent.price1)
        self.priceLabel.text = priceText
        self.buyBtnOutlet.setTitle(jsonContent.actionTitle, for: .normal)
        
        view.backgroundColor = UIColor(hex: jsonContent.bgClr ?? "")
        ...
        buyBtnOutlet.backgroundColor = UIColor(hex: jsonContent.actionTitleClr ?? "")
    }
    ...
    public init() {
        super.init(nibName: "ProductDisplayVC", bundle: Bundle(for: ProductDisplayVC.self))
    }
