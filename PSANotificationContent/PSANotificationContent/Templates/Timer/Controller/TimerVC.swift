...
@objc public class TimerVC: BaseNotificationContentVC {
    ...
    func createView() {
        ...
        if let threshold = jsonContent.timerThreshold {
            thresholdSeconds = threshold
        } else {
            if let endTime = jsonContent.timerEndTime {
                let date = NSDate()
        ...
        if let timerClr = jsonContent.chronoTitleClr, !timerClr.isEmpty {
            timerColor = timerClr
        }
        if let action = jsonContent.deepLink1, !action.isEmpty {
            deeplinkURL = action
        }
        if let bigImg = jsonContent.largeImg, !bigImg.isEmpty {
    ...
    func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            timerLabel.topAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -Utiltiy.getCaptionHeight()),
            ...
            timerLabel.heightAnchor.constraint(equalToConstant: Utiltiy.getCaptionHeight())
        ])
    }
    ...
    func updateViewForExpiredTime() {
        if let jsonContent = jsonContent {
            if let title = jsonContent.altTilte, !title.isEmpty {
                captionLabel.text = title
            }
            if let msg = jsonContent.altMsg, !msg.isEmpty {
                subcaptionLabel.text = msg
            }
            if let altImage = jsonContent.altLargeImg, !altImage.isEmpty {
                // Load expired image, if available.
                Utiltiy.checkImageUrlValid(imageUrl: altImage) { [weak self] (imageData) in
                    DispatchQueue.main.async {
                        if imageData != nil {
                                self?.imageView.image = imageData
                                self?.createFrameWithImage()
    ...
    func activateImageViewContraints() {
        NSLayoutConstraint.activate([
            ...
            imageView.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -Utiltiy.getCaptionHeight())
        ])
    }
